...
(*1. This program read  "config1.txt", "plotdata_config.txt", and read data in /CorrDataDir/CorrDataFile to plot  grid figures*)
...
(*(observable: correlation, sensitivity (dR*correlation) of PDF and residue of data, residue central value, residue uncertainty, (expt error)/(expt central) )*)
...
(**)
(*\:2217 datalis file, Correlation Path, Correlation File, Nx, NQ in "plotdata_config.txt" are setup of filenames that are read in this executable. If the executable need to load PDF data f(x, Q), filenames are determined by F(x,Q) Grid Path, F(x,Q) Grid File, F(x,Q) Samept Path, and F(x,Q) Samept File. *)
(*If Correlation File, F(x,Q) Grid File or F(x,Q) Samept File != "default", data filenames are determined by #PDF set,  Nx, NQ.*)
(**)
...
igroup==1,
If[{a}[[3]]<barmax&&{a}[[3]]>barmin,Setbarcolorfunc2[barcolor,barseperator,{a}[[3]] ],If[{a}[[3]]>=barmax,Red,Blue](*outlayer*)],
(*igroup\[Equal]2: grid data*)Directive[Lighter[Green,0.2], Opacity[0.25]]
]
(*If[ {a}[[3]]<Max[barseperator]&&{a}[[3]]>Min[barseperator],Red,Black]*)
],
...
CorrDataFileGrid="xgrid_data_2017.0425.2123.-0500_ct14nn-new_x80_Q25_corrcut_05_exptid247.m";
(*(x,Q) points sets as position at max of corr*)

If[CorrDataDirGrid\[Equal]"default",CorrDataDirGrid=quickdataDir];
If[CorrDataFileGrid\[Equal]"default",CorrDataFileGrid="grid_data_"<>PDFname<>"_x"<>ToString[GridNx]<>"_Q"<>ToString[GridNQ]<>".m"];
*)
Print["filenames of data"];
Print["Directory: ",CorrDataDir];
Print["corrdataclass: ","corr_"<>CorrDataFile,"\nexpterrordataclass: ","expterror_"<>CorrDataFile,"\nresiduedataclass: ","residue_"<>CorrDataFile,"\ndRdataclass: ","dR_"<>CorrDataFile,"\ndRcorrdataclass: ","dRcorr_"<>CorrDataFile,"\nresidueNsetdataclass: ","residueNset_"<>CorrDataFile,"\n"];
...
residuedataclass=
{Import[CorrDataDir<>"residue_"<>CorrDataFile,"ExpressionML"]};
dRdataclass=
...
{Import[CorrDataDir<>"residueNset_"<>CorrDataFile,"ExpressionML"]};
...
residuedataclassfinal[[1,iexpt,flavour+6]][["data"]]=Delete[residuedataclassfinal[[1,iexpt,flavour+6]][["data"]],listdelete[[iexpt,flavour+6]] ];
dRdataclassfinal[[1,iexpt,flavour+6]][["data"]]=Delete[dRdataclassfinal[[1,iexpt,flavour+6]][["data"]],listdelete[[iexpt,flavour+6]] ];
residueNsetdataclass[[1,iexpt,flavour+6]][["data"]]=Delete[residueNsetdataclass[[1,iexpt,flavour+6]][["data"]],listdelete[[iexpt,flavour+6]] ];
...
p234=processdataplotsmultiexp6percentageHere[{#[[flavour+6]]&/@residuedataclassfinal[[1]]},readcorrconfigfile4[configDir,configfilename],3,flavour];
(*p5=GraphicsGrid[p5,Spacings\[Rule]Scaled[0.15] ];*)
...
(* ::Input:: *)
(*p6=processdataplotsmultiexp6percentageHere[corrdataclassfinal,readcorrconfigfile4[configDir,configfilename],6,6]*)
...
(* ::Input:: *)
(*p5=processdataplotsmultiexp6percentageHere[dRcorrdataclassfinal,readcorrconfigfile4[configDir,configfilename],5,0]*)
...
(* ::Input:: *)
(*processdataplotsmultiexp6percentageHere[{#[[0+6]]&/@expterrordataclassfinal[[1]]},readcorrconfigfile4[configDir,configfilename],2,0]*)
