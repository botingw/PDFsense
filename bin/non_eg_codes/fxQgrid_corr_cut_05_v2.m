...
(**)
(**)
(*\:2217 F(x,Q) Grid Path & F(x,Q) Grid File, Nx, NQ for grid f(x,Q) data*)
(**)
(*\[Dash] output: if Path & File are "default", the program make files with all $obs indexes:*)
(**)
(*\[Dash] ./quick_data/{$obs}_samept_data_{$PDFname}_x{$Nx}_{$NQ}.m, where $PDFname = PDFname, ex: CT14NNLO, $obs = "corr", "dRcorr", "dR", "residue", "residueNset", "expterror"*)
(**)
...
LF[x,Q,expterrorxQ,residuexQ,dRxQ,dRcorrxQ,corrxQ],
...
(*max, min or corr*)
(*
maxcorr=Max[obsgridLFdata[[iexpt,flavour+6,ix]]/.LF[a__]\[RuleDelayed]{a}[[7]] ];
mincorr=Min[obsgridLFdata[[iexpt,flavour+6,ix]]/.LF[a__]\[RuleDelayed]{a}[[7]] ];
*)
corrcut=0.5;
...
If[
maxcorr>Abs[mincorr],
Select[obsgridLFdata[[iexpt,flavour+6,ix]],#[[7]]\[GreaterEqual] 1.0*maxcorr&],
Select[obsgridLFdata[[iexpt,flavour+6,ix]],#[[7]]\[LessEqual] 1.0*mincorr&]
];
*)
];
(*
...
(*
Export[CorrDataDir<>"dRcorr_dRcorrmax_grid_data_"<>PDFname<>"_x"<>ToString[Nx]<>"_Q"<>ToString[NQ]<>".m",dRcorrdataclass,"ExpressionML"];
...
(**)
(*Union[Table[corrdataclass[[iexpt,11]][["data"]]/.LF[a__]:>Take[{a},2],{iexpt,1,Length[corrdataclass]}]//Flatten]*)
...
(*fxQgrid2class[[flavour+6]][["data"]][[600]]/.LF[a__]\[RuleDelayed]Drop[{a},2],*)
(*fxQgrid2class[[flavour+6]][["data"]][[igrid]]/.LF[a__]\[RuleDelayed]Drop[{a},2]*)
(*];*)
...
(* ::Input:: *)
(*Select[obsgridLFdata[[1,5+6,4]],Abs[#[[7]] ]>=0.5&]*)
