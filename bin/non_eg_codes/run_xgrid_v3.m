...
(**)
(*\:2217 datalis file, Correlation Path, Correlation File, Nx in "plotdata_config.txt" are setup of filenames that are read in this executable. If the executable need to load PDF data f(x, Q), filenames are determined by F(x,Q) Grid Path, F(x,Q) Grid File.*)
...
If[CorrDataFileGrid\[Equal]"default",CorrDataFileGrid="xgrid_data_"<>PDFname<>"_x"<>ToString[GridNx]<>".m"];
*)
...
(* ::Input:: *)
(*p234=processdataplotsmultiexp6percentageHere[{residuedataclassfinal[[1]],#[[2+6]]&/@residuedataclassfinal[[2]]},readcorrconfigfile4[configDir,configfilename],3,2]*)
