...
#20171112 botingw: type 1 value is from pointsOrNot[0]
#    configFile.write("Flag:  "+str([1 if i == allradios[1] else 0 for i in range(7)])[1:-1].replace(',','    ')+"\n\n")
    ...
    #20171112 botingw test print
    #print("figtype: ",allradios[1])
...
#20171112 botingw the user define values are put in different file later
#    configFile.write("Name: "+alltexts[4]+"\n")
#    configFile.write("Values: "+alltexts[5]+"\n\n")

    xrangestr = "auto  auto"#20171112 botingw: need to set 2 "auto" in the configure file
    if allchecks[3][0] != 1:
        xrangestr = alltexts[6]+" "+alltexts[7]
    configFile.write("xmin,   xmax:  "+xrangestr+"\n")
    murangestr = "auto  auto"#20171112 botingw: need to set 2 "auto" in the configure file
    if allchecks[4][0] != 1:
    ...
    hxrangestr = "auto  auto"#20171112 botingw: need to set 2 "auto" in the configure file
    if allchecks[6][0] != 1:
...
#20171112 botingw change the highlight range format in config1.txt
...
#    configFile.write("Mode 2 range: "+str([(float(alltexts[2]),float(alltexts[3])) for i in range(7)])[1:-1].replace(',',' ').replace('(',' ').replace(')',' ')+"\n\n")
    ...
    #20171112 botingw test print
    print("test print highlight: (H1) ",alltexts[0],alltexts[1]," (H2) ",alltexts[2],alltexts[3])
    print("t1: ",alltexts[0]," t2: ",alltexts[1])
    print(str([(float(alltexts[0]),float(alltexts[1])) for i in range(7)])[1:-1] )
    print(str([(float(alltexts[0]),float(alltexts[1])) for i in range(7)])[1:-1].replace(',',' ').replace('(',' ').replace(')',' ')+"\n" )
    print(str([(float(alltexts[0]),float(alltexts[1])) for i in range(7)])[1:-1].replace('(','{{ ').replace(')','}} ')+"\n" )
    print("radio: ",allradios[1])
    print("allchecks ",allchecks )
    print("allchecks ",allchecks[1] )
    print("allchecks ",allchecks[2] )

    configFile.close()

#20171112 botingw the user define values are put in different file later
    #Open config file
    configFile = file("./user_define_func.txt","w")
...
def changeR1(one,two,three):
    global range1 
    global xmindefault 
    global xmaxdefault #boting 20171112
    range1.grid_forget()
...
def changeR3(one,two,three):
    ...
    if hdRange[0].get() == 0:
        ...
        texts[10] = rmin3
        rmin3.insert(10,Nbindefault) #boting 20171112
...
def changeR4(one,two,three):
    global range4 
    global histxmindefault 
    global histxmaxdefault #boting 20171112
    range4.grid_forget()
...
root.resizable(width=False,height=False) #20171112 the screen resizale
...
#20171112 botingw: index of figtypes is {2,3,4,5,6,7}, so +1 to value
